...
import { Row, Col, Typography, Tabs, Empty, Pagination } from "antd";
...
const BasicInfo = (props) => {
  ...
  const genderIcon = gender ? (
    <li>{gender === "1" ? <WomanOutlined /> : <ManOutlined />}</li>
  ) : (
    ""
  );
  ...
  const birthplace = place_of_birth ? (
    <li>
      ...
      &nbsp;
      <a
        href={"https://www.google.com/maps/place/" + place_of_birth}
        target="_blank"
  ...
  profile_path = profile_path
    ? CONFIG.IMAGE_SIZE.MEDIUM + profile_path
    : CONFIG.NO_PHOTO.PERSON;
...
const OtherInfo = (props) => {
  ...
  const cast = (
    <CastCrew list={props.profile.movie_credits.cast} currentPage={currPage} />
  );
  const crew = (
    <CastCrew list={props.profile.movie_credits.crew} currentPage={currPage} />
  );
