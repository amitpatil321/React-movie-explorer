...
class ListMovies extends Component {
  ...
  _getMovies() {
    switch (this.props.type) {
      ...
      case "genre":
        this.setState({ isLoading: true });
        API.withGenre(this.props.id, this.props.page)
          .then((response) => {
            this._listMovies(response);
          })
          .catch((error) => {
      ...
      case "discover":
        this.setState({ isLoading: true });
        API.discover(this.props.filters, this.props.page)
          .then((response) => {
            this._listMovies(response);
          })
          .catch((error) => {
  ...
  render() {
    if (this.state.topLoader) return <LoadingOutlined />;
    return (
      <>
        <Row>
          <Col span={24}>{this.props.title && <h2>{this.props.title}</h2>}</Col>
        </Row>
        <br />
        <Row gutter={16}>
          {this.state.error}
          ...
          {this.state.isLoading ? (
            <>
              <LoadingOutlined className="align-center loading-icon" />
              <br />
              <br />
            </>
          ) : (
